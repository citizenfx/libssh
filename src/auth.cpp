                                ...
                                const char *passphrase)
{
    ...
    if (!session->auth_auto_state){
        session->auth_auto_state =
                (ssh_auth_auto_state_struct*)malloc(sizeof(struct ssh_auth_auto_state_struct));
        if (!session->auth_auto_state){
            ssh_set_error_oom(session);
            return SSH_AUTH_ERROR;
        }
...
ssh_kbdint ssh_kbdint_new(void) {
    ...
    kbd = (ssh_kbdint)malloc(sizeof(struct ssh_kbdint_struct));
    if (kbd == NULL) {
        return NULL;
    }
    ZERO_STRUCTP(kbd);
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_info_request) {
  ...
  session->kbdint->prompts = (char**)malloc(nprompts * sizeof(char *));
  ...
  session->kbdint->echo = (unsigned char*)malloc(nprompts);
...
int ssh_userauth_kbdint_setanswer(ssh_session session, unsigned int i,
  ...
  if (session->kbdint->answers == NULL) {
    session->kbdint->answers = (char**)malloc(sizeof(char*) * session->kbdint->nprompts);
    if (session->kbdint->answers == NULL) {
      ssh_set_error_oom(session);
      return -1;
    }
