...
ssh_channel ssh_channel_new(ssh_session session) {
  ...
  channel = (ssh_channel)malloc(sizeof(struct ssh_channel_struct));
  if (channel == NULL) {
...
static int ssh_channel_read_termination(void *s){
  struct ssh_channel_read_termination_struct *ctx = (ssh_channel_read_termination_struct*)s;
...
int ssh_channel_select(ssh_channel *readchans, ssh_channel *writechans,
  ...
  rchans = (ssh_channel*)malloc(sizeof(ssh_channel ) * (count_ptrs(readchans) + 1));
  if (rchans == NULL) {
    return SSH_ERROR;
  }

  wchans = (ssh_channel*)malloc(sizeof(ssh_channel ) * (count_ptrs(writechans) + 1));
  if (wchans == NULL) {
    SAFE_FREE(rchans);
    return SSH_ERROR;
  }

  echans = (ssh_channel*)malloc(sizeof(ssh_channel ) * (count_ptrs(exceptchans) + 1));
  if (echans == NULL) {
    SAFE_FREE(rchans);
    SAFE_FREE(wchans);
    return SSH_ERROR;
