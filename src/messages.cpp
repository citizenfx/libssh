...
static ssh_message ssh_message_new(ssh_session session){
  ssh_message msg = (ssh_message)malloc(sizeof(struct ssh_message_struct));
  if (msg == NULL) {
    return NULL;
  }
  ZERO_STRUCTP(msg);
  ...
  if (strcmp(method, "publickey") == 0) {
    ...
    if(has_sign) {
        ...
        rc = ssh_pki_signature_verify_blob(session,
                                           ...
                                           (unsigned char*)buffer_get_rest(digest),
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_info_response){
  ...
  session->kbdint->answers = (char**)malloc(nanswers * sizeof(char *));
