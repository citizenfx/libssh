...
int pki_key_generate_ed25519(ssh_key key)
{
    ...
    key->ed25519_privkey = (ed25519_privkey*)malloc(sizeof (ed25519_privkey));
    if (key->ed25519_privkey == NULL) {
        goto error;
    }

    key->ed25519_pubkey = (ed25519_pubkey*)malloc(sizeof (ed25519_privkey));
    if (key->ed25519_privkey == NULL) {
        goto error;
    }
                     ...
                     size_t hlen)
{
    ...
    buffer = (uint8_t*)malloc(hlen + ED25519_SIG_LEN);
    if (buffer == NULL) {
        return SSH_ERROR;
    }
    ...
    sig->ed25519_sig = (ed25519_signature*)malloc(ED25519_SIG_LEN);
                       ...
                       size_t hlen)
{
    ...
    buffer2 = (uint8_t*)malloc(hlen + ED25519_SIG_LEN);
    if (buffer2 == NULL) {
        goto error;
    }
 ...
 */
int pki_ed25519_key_dup(ssh_key newk, const ssh_key key)
{
    if (key->ed25519_privkey == NULL || key->ed25519_pubkey == NULL) {
        return SSH_ERROR;
    }

    newk->ed25519_privkey = (ed25519_privkey*)malloc(ED25519_SK_LEN);
    if (newk->ed25519_privkey == NULL) {
        return SSH_ERROR;
    }

    newk->ed25519_pubkey = (ed25519_pubkey*)malloc(ED25519_PK_LEN);
    if (newk->ed25519_privkey == NULL || newk->ed25519_pubkey == NULL){
        SAFE_FREE(newk->ed25519_privkey);
        return SSH_ERROR;
    }

    memcpy(newk->ed25519_privkey, key->ed25519_privkey, ED25519_SK_LEN);
    memcpy(newk->ed25519_pubkey, key->ed25519_pubkey, ED25519_PK_LEN);
