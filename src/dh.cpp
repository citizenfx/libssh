...
int ssh_get_random(void *where, int len, int strong){
  ...
  rng.randomize((uint8_t*)where, len);
...
#elif defined HAVE_BOTAN
	p_group1 = bignum_new();
	bignum_bin2bn(p_group1, P_GROUP1_LEN, p_group1_value);

	p_group14 = bignum_new();
	bignum_bin2bn(p_group14, P_GROUP14_LEN, p_group14_value);
#endif
...
int make_sessionid(ssh_session session) {
...
#ifdef DEBUG_CRYPTO
    ssh_print_hexa("hash buffer", (uint8_t*)ssh_buffer_get_begin(buf), ssh_buffer_get_len(buf));
#endif
    ...
    case SSH_KEX_DH_GROUP14_SHA1:
        ...
        sha1((unsigned char*)buffer_get_rest(buf), buffer_get_rest_len(buf),
    ...
    case SSH_KEX_CURVE25519_SHA256_LIBSSH_ORG:
        ...
        sha256((unsigned char*)buffer_get_rest(buf), buffer_get_rest_len(buf),
                                     session->next_crypto->secret_hash);
        break;
    }
...
static int generate_one_key(ssh_string k,
  ...
  while(requested_size > size) {
    tmp = (unsigned char*)realloc(*output, size + crypto->digest_len);
    if (tmp == NULL) {
      return -1;
    }
    *output = tmp;
...
int generate_session_keys(ssh_session session) {
  ...
  crypto->encryptIV = (unsigned char*)malloc(crypto->digest_len);
  crypto->decryptIV = (unsigned char*)malloc(crypto->digest_len);
  crypto->encryptkey = (unsigned char*)malloc(crypto->digest_len);
  crypto->decryptkey = (unsigned char*)malloc(crypto->digest_len);
  crypto->encryptMAC = (unsigned char*)malloc(crypto->digest_len);
  crypto->decryptMAC = (unsigned char*)malloc(crypto->digest_len);
...
int ssh_get_pubkey_hash(ssh_session session, unsigned char **hash) {
  ...
  *hash = NULL;
  ...
  h = (unsigned char*)malloc(sizeof(unsigned char) * MD5_DIGEST_LEN);
  if (h == NULL) {
    return SSH_ERROR;
  }
    ...
    case SSH_PUBLICKEY_HASH_SHA1:
        {
            ...
            h = (unsigned char*)malloc(SHA_DIGEST_LEN);
            if (h == NULL) {
                rc = -1;
                goto out;
            }
    ...
    case SSH_PUBLICKEY_HASH_MD5:
        {
            ...
            h = (unsigned char*)malloc(MD5_DIGEST_LEN);
            if (h == NULL) {
                rc = -1;
                goto out;
            }
