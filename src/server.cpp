...
static void ssh_server_connection_callback(ssh_session session){
	...
	switch(session->session_state){
		...
		case SSH_SESSION_STATE_DH:
			...
			session->next_crypto->session_id = (unsigned char*)malloc(session->current_crypto->digest_len);
...
enum ssh_publickey_state_e ssh_message_auth_publickey_state(ssh_message msg){
	if (msg == NULL) {
	    return (ssh_publickey_state_e)-1;
	  }
	  return (ssh_publickey_state_e)msg->auth_request.signature_state;
}
...
int ssh_message_auth_interactive_request(ssh_message msg, const char *name,
  ...
  if(num_prompts > 0) {
    msg->session->kbdint->prompts = (char**)malloc(num_prompts * sizeof(char *));
    ...
    msg->session->kbdint->echo = (unsigned char*)malloc(num_prompts * sizeof(unsigned char));
