...
static ssh_buffer gzip_compress(ssh_session session,ssh_buffer source,int level){
  z_stream *zout = (z_stream*)session->current_crypto->compress_out_ctx;
  ...
  if(zout == NULL) {
	zout = (z_stream*)initcompress(session, level);
    session->current_crypto->compress_out_ctx = zout;
    if (zout == NULL) {
      return NULL;
    }
  }
...
static ssh_buffer gzip_decompress(ssh_session session, ssh_buffer source, size_t maxlen) {
  z_stream *zin = (z_stream*)session->current_crypto->compress_in_ctx;
  void *in_ptr = buffer_get_rest(source);
  unsigned long in_size = buffer_get_rest_len(source);
  unsigned char out_buf[BLOCKSIZE] = {0};
  ssh_buffer dest = NULL;
  ...
  if (zin == NULL) {
    zin = (z_stream*)initdecompress(session);
	session->current_crypto->compress_in_ctx = zin;
    if (zin == NULL) {
      return NULL;
    }
  }
