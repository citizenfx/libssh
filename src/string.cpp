...
struct ssh_string_struct *ssh_string_new(size_t size) {
  ...
  str = (ssh_string_struct*)malloc(sizeof(struct ssh_string_struct) + size);
  if (str == NULL) {
    return NULL;
  }
...
struct ssh_string_struct *ssh_string_copy(struct ssh_string_struct *s) {
  ...
  news = ssh_string_new(len);
  if (news == NULL) {
    return NULL;
  }

  memcpy(news->data, s->data, len);
