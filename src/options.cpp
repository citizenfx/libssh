...
int ssh_options_copy(ssh_session src, ssh_session *dest) {
    ...
    if (src->opts.username) {
        news->opts.username = strdup(src->opts.username);
        if (news->opts.username == NULL) {
            ssh_free(news);
            return -1;
        }
    }
    ...
    if (src->opts.host) {
        news->opts.host = strdup(src->opts.host);
        if (news->opts.host == NULL) {
            ssh_free(news);
            return -1;
        }
    }
    ...
    if (src->opts.identity) {
        ...
        news->opts.identity = ssh_list_new();
        if (news->opts.identity == NULL) {
            ssh_free(news);
            return -1;
        }
        ...
        while (it) {
            ...
            rc = ssh_list_append(news->opts.identity, id);
            if (rc < 0) {
                free(id);
                ssh_free(news);
                return -1;
    ...
    if (src->opts.sshdir) {
        news->opts.sshdir = strdup(src->opts.sshdir);
        if (news->opts.sshdir == NULL) {
            ssh_free(news);
            return -1;
        }
    }
    ...
    if (src->opts.knownhosts) {
        news->opts.knownhosts = strdup(src->opts.knownhosts);
        if (news->opts.knownhosts == NULL) {
            ssh_free(news);
            return -1;
        }
    }
    ...
    for (i = 0; i < 10; i++) {
        if (src->opts.wanted_methods[i]) {
            news->opts.wanted_methods[i] = strdup(src->opts.wanted_methods[i]);
            if (news->opts.wanted_methods[i] == NULL) {
                ssh_free(news);
                return -1;
            }
        }
    ...
    if (src->opts.ProxyCommand) {
        news->opts.ProxyCommand = strdup(src->opts.ProxyCommand);
        if (news->opts.ProxyCommand == NULL) {
            ssh_free(news);
            return -1;
        }
    }
    news->opts.fd                   = src->opts.fd;
    news->opts.port                 = src->opts.port;
    news->opts.timeout              = src->opts.timeout;
    news->opts.timeout_usec         = src->opts.timeout_usec;
    news->opts.ssh2                 = src->opts.ssh2;
    news->opts.ssh1                 = src->opts.ssh1;
    news->opts.compressionlevel     = src->opts.compressionlevel;
    news->common.log_verbosity      = src->common.log_verbosity;
    news->common.callbacks          = src->common.callbacks;
    ...
    switch (type) {
        ...
        case SSH_OPTIONS_USER:
            ...
            if (v == NULL) {
                ...
                session->opts.username = strdup(v);
                if (session->opts.username == NULL) {
                    ssh_set_error_oom(session);
                    return -1;
                }
        ...
        case SSH_OPTIONS_COMPRESSION_C_S:
            ...
            if (v == NULL || v[0] == '\0') {
                ...
                if (strcasecmp((const char*)value,"yes")==0){
                    if(ssh_options_set_algo(session,SSH_COMP_C_S,"zlib@openssh.com,zlib") < 0)
                        return -1;
                } else if (strcasecmp((const char*)value,"no")==0){
...
static int ssh_bind_set_key(ssh_bind sshbind, char **key_loc,
    ...
    if (value == NULL) {
        ...
        SAFE_FREE(*key_loc);
        *key_loc = strdup((const char*)value);
        if (*key_loc == NULL) {
  ...
  switch (type) {
    case SSH_BIND_OPTIONS_HOSTKEY:
      if (value == NULL) {
        ...
        return -1;
          ...
          rc = ssh_pki_import_privkey_file((const char*)value, NULL, NULL, NULL, &key);
          if (rc != SSH_OK) {
              return -1;
          }
    ...
    case SSH_BIND_OPTIONS_BINDADDR:
      if (value == NULL) {
        ...
        sshbind->bindaddr = strdup((const char*)value);
    ...
    case SSH_BIND_OPTIONS_BANNER:
      if (value == NULL) {
        ...
        sshbind->banner = strdup((const char*)value);
