...
struct ssh_buffer_struct *ssh_buffer_new(void) {
  struct ssh_buffer_struct *buf = (ssh_buffer_struct*)malloc(sizeof(struct ssh_buffer_struct));
...
static int realloc_buffer(struct ssh_buffer_struct *buffer, size_t needed) {
  ...
  needed = smallest;
  if (buffer->secure){
	  newb = (char*)malloc(needed);
	  if (newb == NULL) {
		  return -1;
      }
	  memcpy(newb, buffer->data,buffer->used);
	  BURN_BUFFER(buffer->data, buffer->used);
	  SAFE_FREE(buffer->data);
  } else {
	  newb = (char*)realloc(buffer->data, needed);
	  if (newb == NULL) {
		  buffer->data = NULL;
		  return -1;
	  }
