...
static int pki_import_pubkey_buffer(ssh_buffer buffer,
    ...
    switch (type) {
        ...
        case SSH_KEYTYPE_ED25519:
        {
            ...
            key->ed25519_pubkey = (ed25519_pubkey*)malloc(ED25519_PK_LEN);
                                 ...
                                 char **b64_key)
{
    ...
    b64 = bin_to_base64((const unsigned char*)ssh_string_data(key_blob), ssh_string_len(key_blob));
...
ssh_string ssh_pki_do_sign(ssh_session session,
    ...
    if (privkey->type == SSH_KEYTYPE_ECDSA) {
        ...
        sig = pki_do_sign(privkey,
                          (const unsigned char*)ssh_buffer_get_begin(buf),
                                         ...
                                         const ssh_key privkey)
{
    ...
    if (privkey->type == SSH_KEYTYPE_ECDSA) {
		...
		*/
        ...
        sig = pki_do_sign_sessionid(privkey, crypto->secret_hash, crypto->digest_len);
        if (sig == NULL) {
            return NULL;
        }
    }
